...
//        param.put("group_id", "94535470");
       F.Promise<WS.Response> resultPromise = VKApi.groups().getById("94535470");
...
// responsePromise.map(result -> ok(views.html.index.render(result.asJson().get("response").toString())));
        ...
        return resultPromise.map(response -> ok(response.asJson() + "text"));
    }
    ...
    public static F.Promise<Result> reactiveComposition() {
        final F.Promise<WS.Response> twitterPromise = WS.url(VKApiConst.API_URL+"http://www.twitter.com").get();
        final F.Promise<WS.Response> typesafePromise = WS.url(VKApiConst.API_URL+"http://www.typesafe.com").get();
